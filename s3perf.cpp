...
testGet( int iconn, int iasyncMan, int key, size_t objectSize )
{
    try
    {
        ...
        s_cons[ iconn ]->get( s_bucketName, getKey( key ).c_str(), s_readBufs[ iconn ], objectSize,
            &response );
...
testPut( int iconn, int iasyncMan, int key, size_t objectSize )
{
    try
    {
        s_cons[ iconn ]->put( s_bucketName, getKey( key ).c_str(), s_writeData, objectSize );
        return true;
    }
    catch( ... )
    {
...
perfTestS3Connection()
{
    ...
    const size_t objectSizes[] = { 1 * MB, 4 * MB, 16 * MB, 64 * MB };
